...
func addNodeFlags(fs *pflag.FlagSet) {
	...
	// OPTIMIZATION: Increased from 2000 to 20000 for 10x larger batches (sync speed improvement)
	fs.Uint(BootstrapAncestorsMaxContainersSentKey, 20000, "Max number of containers in an Ancestors message sent by this node")
	fs.Uint(BootstrapAncestorsMaxContainersReceivedKey, 20000, "This node reads at most this many containers from an incoming Ancestors message")
